...
@router.post("/crypto", summary="Crypto withdrawal")
async def crypto_withdrawal(data: CryptoRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    result = await svc.create_withdrawal(user.id, checking["id"], "crypto", data.amount, 0.0, {"notes": data.notes})
    if not result.get("success"): return APIResponse(success=False, message=result.get("message","Failed"))
    await db.commit()
    return APIResponse(success=True, message="Crypto withdrawal submitted", data=result)

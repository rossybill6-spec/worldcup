...
@router.post("/internal", summary="Internal withdrawal")
async def internal_withdrawal(data: InternalRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    result = await svc.create_withdrawal(user.id, checking["id"], "internal", data.amount, 0.0, {"notes": data.notes})
    if not result.get("success"): return APIResponse(success=False, message=result.get("message","Failed"))
    await db.commit()
    return APIResponse(success=True, message="Internal withdrawal submitted", data=result)

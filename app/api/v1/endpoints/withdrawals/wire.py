...
@router.post("/wire", summary="Wire withdrawal")
async def wire_withdrawal(data: WireRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    result = await svc.create_withdrawal(user.id, checking["id"], "wire", data.amount, 0.0, {"notes": data.notes})
    if not result.get("success"): return APIResponse(success=False, message=result.get("message","Failed"))
    await db.commit()
    return APIResponse(success=True, message="Wire withdrawal submitted", data=result)

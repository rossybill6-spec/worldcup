...
@router.post("/ach", summary="Ach withdrawal")
async def ach_withdrawal(data: AchRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    result = await svc.create_withdrawal(user.id, checking["id"], "ach", data.amount, 0.0, {"notes": data.notes})
    if not result.get("success"): return APIResponse(success=False, message=result.get("message","Failed"))
    await db.commit()
    return APIResponse(success=True, message="Ach withdrawal submitted", data=result)
